...
def check_file(path: str):
    ...
    try:
        ...
        system_state = {
            ...
            "activated": False,
            "trigger_phase_ready": result["verified"],
            "level": 1 if result["verified"] else 0,
            "last_update": datetime.utcnow().isoformat()
        }

        # Vorhandenen Zustand mergen, falls schon vorhanden
        if state_path.exists():
            try:
                old = json.loads(state_path.read_text(encoding="utf-8"))
                old.update(system_state)
                system_state = old
            except Exception:
                pass
        ...
        print("💾 Fortschritt gespeichert – Fortsetzungsmodus aktiv.")
        print(f"📁 {state_path}")

    except Exception as e:
        print(f"[WARN] Konnte system_state.json nicht speichern: {e}")
